    ...
    public class SiwakePrintClass : MyLibrary.MyLoading.Thread
    {
        ...
        public override int MultiThreadMethod()
        {
            ...
            try
            {
                ...
                {
                    var path = MyUtilityModules.AppSetting("roukin_setting", "siwake_root_path");
                    var fileName = MyUtilityModules.AppSetting("roukin_setting", "siwake_name", true);
        ...
        private void Run(List<Report.Models.BankModel> bankModels, StringBuilder maching)
        {
            ...
            foreach (var code in codes)
            {
                ...
                var rows = _table.AsEnumerable().Where(x => x.Field<string>("bpo_bank_code") == code)
                    ...
                    .ThenBy(x => int.Parse(x["taba_count"].ToString()))
                    .CopyToDataTable();
                ...
                {
                    // FixedDocumentを作成
                    document = SiwakeHelper.CreateFixedDocument(rows, code, financial.financial_name);
                });
